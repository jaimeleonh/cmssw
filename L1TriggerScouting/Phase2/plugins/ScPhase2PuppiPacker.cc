        ...
        if (startIdx >= endIdx) {
            ...
            for (unsigned int j = startIdx; j < endIdx; ++j) {
                *output++ = (*puppiCandidates)[j].encodedPuppi64();
                if ((*puppiCandidates)[j].pdgId() == 211) {
                    auto p = (*puppiCandidates)[j];
                    printf("Packing charged hadron: pt=%.2f (%d), eta=%.2f, phi=%.2f, pdgId=%d, puppiw=%.3f, dxy=%.3f, hwdxy=%i\n",
                           p.pt(), p.hwPt(), p.eta(), p.phi(), p.pdgId(), p.puppiWeight(), p.dxy(), p.hwDxy());
                }
