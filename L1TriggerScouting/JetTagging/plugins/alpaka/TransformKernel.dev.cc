...
namespace ALPAKA_ACCELERATOR_NAMESPACE::l1sc::kernels {
  ...
  public:
    ...
    ALPAKA_FN_ACC void operator()(
        ...
        VertexDeviceCollection::ConstView vertices,
        BxLookupSoA::ConstView vertexBxLookup,
      ...
      for (uint32_t bx_idx : independent_groups(acc, grid_dim)) {
        uint32_t vertex_begin = vertexBxLookup.offset()[bx_idx].offset();
        uint32_t vertex_end = vertexBxLookup.offset()[bx_idx + 1].offset();
        if (vertex_end <= vertex_begin)
          continue;
        auto& vertex_z0 = alpaka::declareSharedVar<float, __COUNTER__>(acc);
        if (once_per_block(acc)) {
          vertex_z0 = vertices.z0()[vertex_begin];  // just need the first vertex
        }

        uint32_t jet_begin = jetBxLookup.offset()[bx_idx].offset();
        uint32_t jet_end = jetBxLookup.offset()[bx_idx + 1].offset();
        if (jet_end <= jet_begin)
          continue;
        uint32_t njets = jet_end - jet_begin;

        for (auto jet_idx : independent_group_elements(acc, njets)) {
        // for (auto block_idx : independent_groups(acc, clusters_cands.offset().metadata().size() - 1)) {
          auto block_idx = jet_begin + jet_idx;
          ...
          for (auto tid : independent_group_elements(acc, input_size)) {
            ...
            input_tensor.features()(tid, 7) = pf.z0()[p] - vertex_z0;
  ...
  SoftJetInputDeviceTensor transform(Queue& queue, 
    ...
    unsigned int nbx = jetBxLookup.const_view().offset().metadata().size() - 1;
    unsigned int nbx_v = vertexBxLookup.const_view().offset().metadata().size() - 1;
