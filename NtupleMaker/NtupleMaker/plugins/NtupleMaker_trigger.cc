...
vector<int>	hltL1DoubleTauJet_nJets;
...
vector<int>	hltL1DoubleTauJet_nTaus;
...
void NtupleMaker::fillTriggers(const edm::Event& iEvent){
    ...
    // 2 Tau + Jet
    std::string pathNameDoubleTauJet = "HLT_DoubleMediumChargedIsoPFTauHPS30_Trk1_eta2p1_Reg_PFJet60";
    ...
    if (nJets > 0) {
        hltL1DoubleTauJet_nJets.push_back(nJets);
    ...
    if (nTaus > 0) {
        hltL1DoubleTauJet_nTaus.push_back(nTaus);
    ...
    for(trigger::size_type iFilter=0; iFilter!=nFilters; ++iFilter) {
        ...
        for(trigger::size_type iKey=0; iKey < nObjKeys; ++iKey){
            ...
            if (filterTag == hltHpsDoublePFTau30TrackPt1MediumChargedIsolationAndTightOOSCPhotonsDz02Reg_Tag
                ...
                HLT1PFJetFilter_eta.push_back(pt_);
