...
void MuonPathAnalyzerPerSL::run(edm::Event& iEvent, const edm::EventSetup& iEventSetup, std::vector<MuonPath*> &muonpaths, std::vector<MuonPath*> &outmuonpaths) {
  if (debug) cout <<"MuonPathAnalyzerPerSL: run" << endl;
...
bool MuonPathAnalyzerPerSL::isCompatibleWithGrouping(MuonPath *mpath, int SL){
  
  int SLit=0;
  if (SL==2) SLit=2;

  for (int i=SLit*2; i<SLit*2+3; i++) {
    cout << "LAT from PRIM: " << mpath->getPrimitive(i)->getLaterality() << " = " << (mpath->getLateralComb(SL))[i-SLit*2] << endl;
    if (mpath->getPrimitive(i)->getLaterality() == NONE) continue;
    if (mpath->getPrimitive(i)->getLaterality() != (mpath->getLateralComb(SL))[i-SLit*2])
      return false;
  }
  return true;
}
...
void MuonPathAnalyzerPerSL::evaluatePathQuality(MuonPath *mPath, int SL) {

    int totalHighQ = 0, totalLowQ = 0;
  
    if(debug) std::cout<<"DTp2:evaluatePathQuality \t\t\t\t\t En evaluatePathQuality Evaluando PathQ. Celda base: "<<mPath->getBaseChannelId(SL)<<std::endl;
    if(debug) std::cout<<"DTp2:evaluatePathQuality \t\t\t\t\t Total lateralidades: "<<totalNumValLateralities<<std::endl;

    // Por defecto.
    mPath->setQuality(NOPATH,SL);
     ...
     */
    if (totalHighQ == 1) {
      mPath->setQuality(HIGHQ,SL);
    }
    else if (totalHighQ > 1) {
      mPath->setQuality(HIGHQGHOST,SL);
    }
    else if (totalLowQ == 1) {
      mPath->setQuality(LOWQ,SL);
    }
    else if (totalLowQ > 1) {
      mPath->setQuality(LOWQGHOST,SL);
    }
}
 ...
 * de las capas 0 y 2.
 */
void MuonPathAnalyzerPerSL::calculatePathParameters(MuonPath* mPath, int SL) {
    ...
    calcCellDriftAndXcoor(mPath,SL);
    //calcTanPhiXPosChamber(mPath);
    if(debug) std::cout<<"DTp2:calculatePathParameters \t\t\t\t\t\t  checking mPath->getQuality() "<<mPath->getQuality(SL)<<std::endl;
    if (mPath->getQuality(SL) == HIGHQ or mPath->getQuality(SL) == HIGHQGHOST){
	if(debug) std::cout<<"DTp2:calculatePathParameters \t\t\t\t\t\t\t  Quality test passed, now calcTanPhiXPosChamber4Hits(mPath) "<<std::endl;
	calcTanPhiXPosChamber4Hits(mPath,SL);
    }else{
	if(debug) std::cout<<"DTp2:calculatePathParameters \t\t\t\t\t\t\t  Quality test NOT passed calcTanPhiXPosChamber3Hits(mPath) "<<std::endl;
	calcTanPhiXPosChamber3Hits(mPath,SL);
    }
    if(debug) std::cout<<"DTp2:calculatePathParameters \t\t\t\t\t\t calcChiSquare(mPath) "<<std::endl;
    calcChiSquare(mPath,SL);
}

void MuonPathAnalyzerPerSL::calcTanPhiXPosChamber(MuonPath* mPath, int SL)
{
    /*
    ...
    */
    ...
    if (mPath->getPrimitive(L1)->isValidTime()) layerIdx[0] = 0;
    else layerIdx[0] = 1;

    if (mPath->getPrimitive(L4)->isValidTime()) layerIdx[1] = 3;
    else layerIdx[1] = 2;

    /* We identify along which cells' sides the muon travels */
    LATERAL_CASES sideComb[2];
    sideComb[0] = (mPath->getLateralComb(SL))[ layerIdx[0] ];
    sideComb[1] = (mPath->getLateralComb(SL))[ layerIdx[1] ];

    /* Horizontal gap between cells in cell's semi-length units */
    int dHoriz = (mPath->getCellHorizontalLayout(SL))[ layerIdx[1] ] -
	(mPath->getCellHorizontalLayout(SL))[ layerIdx[0] ];
    ...
    */
    ...
    mPath->setHorizPos( lowerXPHorizPos + lowerXPVertPos * tanPhi , SL);
}

/**
 * Cálculos de coordenada y ángulo para un caso de 4 HITS de alta calidad.
 */
void MuonPathAnalyzerPerSL::calcTanPhiXPosChamber4Hits(MuonPath* mPath, int SL) {
  int L1(0),L2(1),L3(2),L4(3);
  if (SL==2) {L1=4; L2=5; L3=6; L4=7;} 
  
  float tanPhi = (3 * mPath->getXCoorCell(L4) +
		  mPath->getXCoorCell(L3) -
		  mPath->getXCoorCell(L2) -
		  3 * mPath->getXCoorCell(L1)) / (10 * CELL_HEIGHT);

    mPath->setTanPhi(tanPhi,SL);

    float XPos = (mPath->getXCoorCell(L1) +
		  mPath->getXCoorCell(L2) +
		  mPath->getXCoorCell(L3) +
		  mPath->getXCoorCell(L4)) / 4;

    mPath->setHorizPos( XPos , SL);
}

/**
 * Cálculos de coordenada y ángulo para un caso de 3 HITS.
 */
void MuonPathAnalyzerPerSL::calcTanPhiXPosChamber3Hits(MuonPath* mPath, int SL) {
    int L1(0),L2(1),L3(2),L4(3);
    if (SL==2) {L1=4; L2=5; L3=6; L4=7;}
    ...
    /*-----------------------------------------------------------------*/
    float XPos = 0;
    if (mPath->getPrimitive(L1)->isValidTime() and
	mPath->getPrimitive(L4)->isValidTime())
	XPos = (mPath->getXCoorCell(L1) + mPath->getXCoorCell(L4)) / 2;
    else
	XPos = (mPath->getXCoorCell(L2) + mPath->getXCoorCell(L3)) / 2;
 ...
 */
void MuonPathAnalyzerPerSL::calcCellDriftAndXcoor(MuonPath *mPath, int SL) {
	...
	if (mPath->getPrimitive(i)->isValidTime()) {
	    ...
	    if ( (mPath->getLateralComb(SL))[ i ] == LEFT )
 ...
 */
void MuonPathAnalyzerPerSL::calcChiSquare(MuonPath *mPath,int SL) {

    float xi, zi, factor;

    float chi = 0;
    float mu  = mPath->getTanPhi(SL);
    float b   = mPath->getHorizPos(SL);
...
//  cout << "first clone: " << mPath << endl;
//  cout << mPath->getNPrimitives() << endl;
//  cout << mPath->getPrimitive(0) << " " << mPath->getPrimitive(1) << " " << mPath->getPrimitive(2) << " " << mPath->getPrimitive(3) << endl;
...
//	cout << "Second clone: " << mpAux << endl;
//	cout << mpAux->getNPrimitives() << endl;
//	cout << mpAux->getPrimitive(0) << " " << mpAux->getPrimitive(1) << " " << mpAux->getPrimitive(2) << " " << mpAux->getPrimitive(3) << endl;
...
//  void MuonPathAnalyzerPerSL::analyze(MuonPath *inMPath, MuonPath *outMPath, int SL){
//  }

void MuonPathAnalyzerPerSL::analyze(MuonPath *inMPath, std::vector<MuonPath*> &outMPath, int SL){
  if(debug) std::cout<<"DTp2:analyze \t\t\t\t starts"<<std::endl;
  if(debug) std::cout<<"DTp2:analyze \t\t\t\t In analyze function checking if inMPath->isAnalyzable() "<<inMPath->isAnalyzable()<<std::endl; 
  ...
  // LOCATE MPATH:
  int selected_Id=0;
  if (inMPath->getNPrimitives()==4 or (inMPath->getNPrimitives()>4 and SL==0)) {
    if     (inMPath->getPrimitive(0)->getTDCTime()!=-1) selected_Id=inMPath->getPrimitive(0)->getCameraId();
    else if(inMPath->getPrimitive(1)->getTDCTime()!=-1) selected_Id=inMPath->getPrimitive(1)->getCameraId(); 
    else if(inMPath->getPrimitive(2)->getTDCTime()!=-1) selected_Id=inMPath->getPrimitive(2)->getCameraId(); 
    else if(inMPath->getPrimitive(3)->getTDCTime()!=-1) selected_Id=inMPath->getPrimitive(3)->getCameraId(); 
  }
  else if (inMPath->getNPrimitives()>4 and SL==2) {
    if     (inMPath->getPrimitive(4)->getTDCTime()!=-1) selected_Id=inMPath->getPrimitive(4)->getCameraId();
    else if(inMPath->getPrimitive(5)->getTDCTime()!=-1) selected_Id=inMPath->getPrimitive(5)->getCameraId(); 
    else if(inMPath->getPrimitive(6)->getTDCTime()!=-1) selected_Id=inMPath->getPrimitive(6)->getCameraId(); 
    else if(inMPath->getPrimitive(7)->getTDCTime()!=-1) selected_Id=inMPath->getPrimitive(7)->getCameraId(); 
  }
  ...
  // Clonamos el objeto analizado.
  MuonPath *mPath = new MuonPath(inMPath); 
//  cout << "SFG: first clone " << mPath << endl;
//  for (int i=0; i<mPath->getNPrimitives(); i++) 
//    cout << mPath->getPrimitive(i)->getTDCTime()  << " ("<< mPath->getPrimitive(i)->getChannelId() << ");  " ;
//  cout << endl;
  
  if (mPath->isAnalyzable(SL)) {
    if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t yes it is analyzable "<<mPath->isAnalyzable(SL)<<std::endl;
    setCellLayout( mPath->getCellHorizontalLayout(SL) );
    evaluatePathQuality(mPath,SL);
  }else{
    if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t no it is NOT analyzable "<<mPath->isAnalyzable(SL)<<std::endl;
  }


  if(debug) std::cout<<"DTp2:analyze \t\t\t\t checking if it passes the min quality cut "<<mPath->getQuality(SL)<<">"<<minQuality<<std::endl;
  if ( mPath->getQuality(SL) >= minQuality ) {
    if(debug) std::cout<<"DTp2:analyze \t\t\t\t min quality achievedCalidad: "<<mPath->getQuality(SL)<<std::endl;
    
    for (int i = SLit*2; i <= SLit*2+3; i++){
      if(debug) std::cout <<"DTp2:analyze \t\t\t\t  Capa: " << mPath->getPrimitive(i)->getLayerId()
			  <<" Canal: " << mPath->getPrimitive(i)->getChannelId() 
			  <<" TDCTime: " << mPath->getPrimitive(i)->getTDCTime() 
			  << std::endl;
    }
    ...
    for (int i = 0; i < totalNumValLateralities; i++) {//here
      if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t laterality #- "<<i<<std::endl;
      if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t laterality #- "<<i<<" checking quality:"<<std::endl;
      if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t laterality #- "<<i<<" checking mPath Quality="<<mPath->getQuality(SL)<<std::endl;   
      if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t laterality #- "<<i<<" latQuality[i].val="<<latQuality[i].valid<<std::endl;   
      if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t laterality #- "<<i<<" before if:"<<std::endl;
      
      if (latQuality[i].valid and (((mPath->getQuality(SL)==HIGHQ or mPath->getQuality(SL)==HIGHQGHOST) and latQuality[i].quality==HIGHQ)
				   or
				   ((mPath->getQuality(SL) == LOWQ or mPath->getQuality(SL)==LOWQGHOST) and latQuality[i].quality==LOWQ))){
...
//	  cout << mpAux->getPrimitive(i)->getTDCTime()  << " ("<< mpAux->getPrimitive(i)->getChannelId() << ");  " ;
//	cout << endl;
	...
	if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t  calculating parameters "<<std::endl;
	calculatePathParameters(mpAux,SL);
	 ...
	 * Y pasamos al siguiente elemento.
	 */
	if ((mpAux->getQuality(SL) == HIGHQ or mpAux->getQuality(SL) == HIGHQGHOST) && mpAux->getChiSq(SL) > chiSquareThreshold) {//check this if!!!
	  if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t  HIGHQ or HIGHQGHOST but min chi2 or Q test not satisfied "<<std::endl;
	}				
	else{
	  if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t  inside else, returning values: "<<std::endl;
	  if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t  BX Time = "<<mpAux->getBxTimeValue(SL)<<std::endl;
	  if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t  BX Id   = "<<mpAux->getBxNumId(SL)<<std::endl;
	  if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t  XCoor   = "<<mpAux->getHorizPos(SL)<<std::endl;
	  if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t  tan(Phi)= "<<mpAux->getTanPhi(SL)<<std::endl;
	  if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t  chi2= "<<mpAux->getChiSq(SL)<<std::endl;
	  if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t  lateralities = "
			     <<" "<<mpAux->getLateralComb(SL)[0]
			     <<" "<<mpAux->getLateralComb(SL)[1]
			     <<" "<<mpAux->getLateralComb(SL)[2]
			     <<" "<<mpAux->getLateralComb(SL)[3]
			     <<std::endl;
	  
	  //changing to chamber frame or reference:	  
	  DTChamberId ChId(MuonPathSLId.wheel(),MuonPathSLId.station(),MuonPathSLId.sector());
	  
	  float jm_tanPhi=-1.*mpAux->getTanPhi(SL); //testing with this line
	  float jm_x=(mpAux->getHorizPos(SL)/10.)+shiftinfo[wireId.rawId()]; 
	  //0.65 is half hight of a cell needed to go to the middle of the superlayer, here we are extrapolating with the angle of the primitive!
	  jm_x=jm_x-(zinfo[wireId.rawId()]-0.65)*jm_tanPhi; 
	  float jm_t0=mpAux->getBxTimeValue(SL);		      
	  MP_QUALITY quality=mpAux->getQuality(SL); 
	  ...
	  if(quality == HIGHQ or quality == HIGHQGHOST){ 
	    if(chi2<best_chi2){
	      chi2_jm_tanPhi=jm_tanPhi;
	      chi2_jm_x=(mpAux->getHorizPos(SL)/10.)+shiftinfo[wireId.rawId()]; 
	      chi2_jm_x=chi2_jm_x-(zinfo[wireId.rawId()]-0.65)*chi2_jm_tanPhi; //from SL to CH
	      chi2_jm_t0=mpAux->getBxTimeValue(SL);		      
	      chi2_phi=phi;
	      chi2_phiB=phiB;
	      chi2_chi2=chi2*0.01; //in cmssw we want cm, 1 cm^2 = 100 mm^2
	      chi2_quality= mpAux->getQuality(SL);
	      best_chi2=mpAux->getChiSq(SL);
	    }
	  } 
	  else {//write the metaprimitive in case no HIGHQ or HIGHQGHOST
	    if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t\t\t\t  pushing back metaprimitive no HIGHQ or HIGHQGHOST"<<std::endl;
	    ...
	    //Loading Info into mpAux
	    mpAux->setBxTimeValue(jm_t0,SL);
 	    mpAux->setHorizPos(jm_x,SL);
 	    mpAux->setTanPhi(jm_tanPhi,SL);	    
 	    mpAux->setPhi(phi,SL);
 	    mpAux->setPhiB(phiB,SL);
 	    mpAux->setChiSq(chi2*0.01,SL);
	    mpAux->setQuality(quality,SL);
	    mpAux->setRawId(thisLId.rawId());
	...
	if(debug) std::cout<<"DTp2:analyze \t\t\t\t\t\t\t\t  latQuality[i].valid and (((mpAux->getQuality(SL)==HIGHQ or mpAux->getQuality()==HIGHQGHOST) and latQuality[i].quality==HIGHQ) or  ((mpAux->getQuality() == LOWQ or mpAux->getQuality()==LOWQGHOST) and latQuality[i].quality==LOWQ)) not passed"<<std::endl;
      }
    }
    ...
    if(chi2_jm_tanPhi!=999){//
      ...
      //Loading Info into mpAux
      mPath->setBxTimeValue(chi2_jm_t0,SL);
      mPath->setChiSq(chi2_chi2,SL);
      mPath->setPhi(chi2_phi,SL);
      mPath->setPhiB(chi2_phiB,SL);
      mPath->setTanPhi(chi2_jm_tanPhi,SL);
      mPath->setHorizPos(chi2_jm_x,SL);
      mPath->setQuality(chi2_quality,SL);
      mPath->setRawId(thisLId.rawId());
      
      outMPath.push_back(std::move(mPath));
    }
  }
  //  delete mpAux;
  if(debug) std::cout<<"DTp2:analyze \t\t\t\t finishes"<<std::endl;
