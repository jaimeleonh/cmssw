...
void HoughGrouping::run(edm::Event& iEvent, const edm::EventSetup& iEventSetup, DTDigiCollection digis, std::vector<MuonPath*> *outMpath) {
  if (debug) cout << "HoughGrouping::run" << endl;
  ...
  for (DTDigiCollection::DigiRangeIterator dtLayerIdIt = digis.begin();           dtLayerIdIt != digis.end();              dtLayerIdIt++) {
    ...
    for (DTDigiCollection::const_iterator  digiIt = ((*dtLayerIdIt).second).first; digiIt != ((*dtLayerIdIt).second).second; digiIt++) {
      if (debug) {
        cout << "HoughGrouping::run - Digi number " << idigi                             << endl;
      ...
      if (debug) {
        cout << "HoughGrouping::run - Indice del primer wire de la lay: "   << lay->specificTopology().firstChannel() << endl;
        cout << "HoughGrouping::run - Indice del last wire de la lay: "     << lay->specificTopology().lastChannel()  << endl;
        cout << "HoughGrouping::run - Anchu la celda: "                     << lay->specificTopology().cellWidth()    << endl;
        cout << "HoughGrouping::run - Altu la celda: "                      << lay->specificTopology().cellHeight()   << endl;
        cout << "HoughGrouping::run - Pos. (x) del primer wire de la lay: " << lay->specificTopology().wirePosition(lay->specificTopology().firstChannel()) << endl;
        cout << "HoughGrouping::run - Pos. (x) del last wire de la lay: "   << lay->specificTopology().wirePosition(lay->specificTopology().lastChannel()) << endl;
...
//       nhits += 1;
  ...
  if (debug) {
    cout << "HoughGrouping::run - nhits nesti evento"     << nhits << endl;
    cout << "HoughGrouping::run - nseldigis nesti evento" << idigi << endl;
  }
  
  if (hitvec.size() == 0) {
    cout << "HoughGrouping::run - No digis present in this event" << nhits << endl;
    outMpath->clear();
    return;
  }
...
//   cout << "Number with 6: " << tmpvec.size() << endl;
//   cout << "\nBEFORE" << endl;
//   for (UShort_t i = 0; i < tmpvec.size(); i++) {
//     if (get<0>(tmpvec.at(i)) < TMath::PiOver2()) cout << "   ang (rad): " << get<0>(tmpvec.at(i)) << ", ang (º): " << get<0>(tmpvec.at(i)) * 360/TMath::TwoPi()       << ", pos: " << get<1>(tmpvec.at(i)) << endl;
//     else                                         cout << "   ang (rad): " << get<0>(tmpvec.at(i)) << ", ang (º): " << 180 - get<0>(tmpvec.at(i)) * 360/TMath::TwoPi() << ", pos: " << get<1>(tmpvec.at(i)) << endl;
//   }
...
void HoughGrouping::OrderAndFilter(std::vector<std::tuple<UShort_t, Bool_t*, Bool_t*, UShort_t, Double_t*, DTPrimitive*>> invector, std::vector<MuonPath*> *outMuonPath) {
  ...
  cout << "At the end, we have only " << invector.size() << " good paths!" << endl;
  cout << "Let's draw them! We also will print in terminal info of them" << endl;
