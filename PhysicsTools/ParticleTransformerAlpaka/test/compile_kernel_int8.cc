...
// original row-major INT8 weights and folds the per-channel scale into every
// product, the fast kernel reads the transposed image and applies the scale
// once per output.
#include <algorithm>
#include <barrier>
#include <chrono>
...
#include "PhysicsTools/ParticleTransformerAlpaka/interface/alpaka/ParticleTransformerKernel.h"
...
std::vector<std::int8_t> readWeights(char const* path, std::size_t expected) {
  std::ifstream input(path, std::ios::binary | std::ios::ate);
  if (!input || static_cast<std::size_t>(input.tellg()) != expected)
    throw std::runtime_error("weight file has the wrong size");
  input.seekg(0);
  std::vector<std::int8_t> values(expected);
  input.read(reinterpret_cast<char*>(values.data()), static_cast<std::streamsize>(values.size()));
  if (!input)
    throw std::runtime_error("weight file read failed");
  return values;
}
...
void runReference(part::generated::ModelView model,
                  float const* features,
                  float const* vectors,
                  bool const* mask,
                  float* probabilities) {
  part::Kernel{}(Acc{}, model, features, vectors, mask, probabilities);
}

void runFast(part::generated::ModelView model,
             float const* features,
             float const* vectors,
             bool const* mask,
             float* probabilities,
             uint32_t blockThreads) {
  auto shared = std::make_unique<part::JetShared>();
  std::memset(shared.get(), 0, sizeof(part::JetShared));
  std::copy_n(features, part::kMaxParticles * part::kInputFeatures, shared->na);
  std::copy_n(vectors, part::kMaxParticles * part::kFourVector, shared->p4);
  for (int particle = 0; particle < part::kMaxParticles; ++particle)
  ...
  std::barrier blockBarrier(static_cast<std::ptrdiff_t>(blockThreads));
  {
    ...
    for (uint32_t lane = 0; lane < blockThreads; ++lane)
      threads.emplace_back([&, lane] {
        part::runParticleTransformer(Acc{lane, blockThreads, &blockBarrier, nullptr}, model, *shared);
      });
  }
...
int main(int argc, char** argv) {
  if (argc != 3) {
    std::cerr << "usage: compile_kernel_int8 WEIGHTS PARAMS\n";
    return 2;
  }
  auto weights = readWeights(argv[1], part::generated::weights_size);
  auto params = readFloats(argv[2], part::generated::params_size);

  std::vector<std::int8_t> transposed(part::generated::weights_size, 0);
  auto const covered = part::transposeWeights(weights.data(), params.data(), transposed.data());
  if (covered != part::generated::weights_size) {
    std::cerr << "the tensor walk covered " << covered << " of " << part::generated::weights_size
              << " weights\n";
    return 1;
  }
  std::cout << "shared bytes per jet: " << sizeof(part::JetShared) << "\n";
  if (sizeof(part::JetShared) > 48u * 1024u) {
    std::cerr << "the shared footprint exceeds the 48 kB static limit\n";
    return 1;
  }

  part::generated::ModelView reference{weights.data(), params.data()};
  part::generated::ModelView fast{transposed.data(), params.data()};
  ...
  runFast(fast, features, vectors, mask, fastProbabilities, part::kThreadsPerJet);
  ...
  // Quantized inference is not bitwise stable across reduction orders: the
  // block kernel's LayerNorm sums in a different order from the scalar
  // reference, and a difference of a few 1e-7 before rounding occasionally
  // moves an activation across an INT8 boundary.  Each such flip shifts one
  // input by one quantization step, so the tolerance here is a few 1e-3 rather
  // than the 1e-7 of the FP32 build.  Roughly one or two activations flip per
  // jet out of about 190,000.
  constexpr float kTolerance = 3.e-3f;
  bool valid = std::abs(referenceProbabilities[0] - 0.22849616f) < 1.e-5f &&
               std::abs(fastProbabilities[0] - referenceProbabilities[0]) < kTolerance &&
               std::abs(fastProbabilities[1] - referenceProbabilities[1]) < kTolerance;
