...
void TriggerObjectTableProducer::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  unsigned int nobj = l1Jet.size(0) + l1Tau.size(0) + l1Muon.size(0) + l1EG.size(0);
  ...
  objtab->addColumn<float>("pt", l1pt, "pt", 12);
  objtab->addColumn<float>("eta", l1eta, "eta", 12);
  objtab->addColumn<float>("phi", l1phi, "phi", 12);
  objtab->addColumn<int>("iso", l1iso, "iso");
  objtab->addColumn<int>("type", l1type, "type");
