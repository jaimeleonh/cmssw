...
reference is covered by test/compile_kernel_fp32.cc.
"""
...
def test_shared_footprint_fits_both_back_ends():
    ...
    assert raw == 43772
    assert total == 43776
    assert total <= 44 * 1024        # the header's own budget
...
def test_source_contract():
    ...
    algo = (package / "plugins/alpaka/ParticleTransformerFP32Algo.dev.cc").read_text()
    header = (package / "plugins/alpaka/ParticleTransformerFP32Algo.h").read_text()
    producer = (package / "plugins/alpaka/ParticleTransformerFP32Producer.cc").read_text()
    ...
    config = (package / "python/particleTransformerFP32Producer_cfi.py").read_text()
    ...
    assert "declareSharedVar<::partfp32::JetShared" in algo
    ...
    assert "PARTICLE_TRANSFORMER_FP32_ALGO_API_REVISION 5" in header
    assert "PARTICLE_TRANSFORMER_FP32_ALGO_API_REVISION != 5" in algo
    ...
    assert "reinterpret_cast<Float4 const*>(base + 4 * q)" in kernel
    assert "alignas(16) float na[kNormScratch]" in kernel
    assert kernel.count("gemmTiled<") == kernel.count("gemmTiled<")
    assert kernel.count(", true>(") + kernel.count(", true, true>(") >= 6
