"""Full-precision NumPy reference for the b_kinadd Weaver ParT graph."""
...
class WeaverParT:
    def __init__(self,state,model_variant="legacy_mha",pair_final_gelu=None):
    ...
    def pair(self,v):
        ...
        pt=np.sqrt(np.maximum(px*px+py*py,1e-8)); rap=.5*np.log(1+2*pz/np.maximum(e-pz,1e-20)); phi=np.arctan2(py,px)
    ...
    def block(self,x,prefix,bias=None,cls=False,mask=None):
        if cls:
            ...
            # Weaver class attention normalizes the concatenated K/V tokens,
            # but deliberately projects Q from the unnormalized class token.
        ...
        if self.model_variant=="legacy_mha":
            # Exact semantics of einsum('tbhd,h->tbdh') in the legacy Block.
