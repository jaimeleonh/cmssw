...
#if !defined(PARTICLE_TRANSFORMER_FP32_ALGO_API_REVISION) || PARTICLE_TRANSFORMER_FP32_ALGO_API_REVISION != 5
#error "ParticleTransformerFP32Algo.h is stale: replace the complete ParticleTransformerAlpakaFP32 package before rebuilding"
#endif
...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  namespace {
    ...
    public:
      ...
      ALPAKA_FN_ACC void operator()(TAcc const& acc,
        ...
        constexpr uint32_t particles = ::partfp32::kMaxParticles;
        constexpr uint32_t features = ::partfp32::kInputFeatures;
        constexpr uint32_t components = ::partfp32::kFourVector;
        ...
        for (uint32_t jet = block; jet < numberOfJets; jet += blocks) {
          ...
          ::partfp32::runParticleTransformer(acc, model, shared);
  ...
  void ParticleTransformerFP32Algo::run(Queue& queue,
                                        float const* weights,
                                        float const* params,
    ...
    auto const workDiv = cms::alpakatools::make_workdiv<Acc1D>(blocks, ::partfp32::kThreadsPerJet);
    alpaka::exec<Acc1D>(queue,
                        workDiv,
                        JetInferenceKernel{},
                        ::partfp32::generated::ModelView{weights, params},
                        inputs.const_view(),
                        outputs.view(),
                        numberOfJets);
  }
