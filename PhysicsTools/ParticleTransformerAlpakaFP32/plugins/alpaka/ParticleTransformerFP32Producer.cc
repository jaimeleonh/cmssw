...
#include "PhysicsTools/ParticleTransformerAlpakaFP32/plugins/alpaka/ParticleTransformerFP32Algo.h"
...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  namespace {
    ...
    // The transposed model is 8.4 MB and is read by every block of every
    // inference launch.  One copy per CMSSW stream would multiply the L2
    // working set by the number of streams sharing a GPU and turn the weight
    // stream into the bottleneck, so the buffers are owned per *device* and
    // shared by all streams that run on it.
    struct ResidentModel {
      DeviceFloats weights;
      DeviceFloats params;
    };
    ...
    void readExact(edm::FileInPath const& file, T* destination, std::size_t count) {
      ...
      if (!input || static_cast<std::size_t>(input.tellg()) != expectedBytes) {
        throw cms::Exception("ParticleTransformerFP32Model")
      ...
      if (!input) {
        throw cms::Exception("ParticleTransformerFP32Model") << "Could not read model file " << file.fullPath();
      }
    }
    ...
    std::shared_ptr<ResidentModel> residentModel(Queue& queue,
      ...
      auto rowMajor = cms::alpakatools::make_host_buffer<float[]>(queue, ::partfp32::generated::weights_size);
      auto columnMajor = cms::alpakatools::make_host_buffer<float[]>(queue, ::partfp32::generated::weights_size);
      auto parameters = cms::alpakatools::make_host_buffer<float[]>(queue, ::partfp32::generated::params_size);
      readExact(weightsFile, rowMajor.data(), ::partfp32::generated::weights_size);
      readExact(paramsFile, parameters.data(), ::partfp32::generated::params_size);

      // Tensors that are not dense weights (norm scales, batch-norm
      // statistics, the class token) keep their exported values, so start from
      // a verbatim copy and let the transposition overwrite the dense blocks.
      std::copy_n(rowMajor.data(), ::partfp32::generated::weights_size, columnMajor.data());
      auto const covered = ::partfp32::transposeWeights(rowMajor.data(), columnMajor.data());
      if (covered != ::partfp32::generated::weights_size) {
        throw cms::Exception("ParticleTransformerFP32Model")
            << "The transposed model covers " << covered << " of " << ::partfp32::generated::weights_size
            << " floats; the weight file does not match GeneratedModel.h";
      }

      auto model = std::make_shared<ResidentModel>(ResidentModel{
          cms::alpakatools::make_device_buffer<float[]>(queue, ::partfp32::generated::weights_size),
          cms::alpakatools::make_device_buffer<float[]>(queue, ::partfp32::generated::params_size)});
  ...
  class ParticleTransformerFP32Producer : public stream::EDProducer<> {
  public:
    explicit ParticleTransformerFP32Producer(edm::ParameterSet const& params)
        : EDProducer<>(params),
    ...
    static void fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
      ...
      desc.add<std::string>("weightsFile", "PhysicsTools/ParticleTransformerAlpakaFP32/data/model_weights.fp32.bin");
      desc.add<std::string>("paramsFile", "PhysicsTools/ParticleTransformerAlpakaFP32/data/model_params.fp32.bin");
    ...
    void produce(device::Event& event, device::EventSetup const&) override {
      ...
      auto inputs = partfp32::makeParticleTransformerFP32Inputs(
  ...
  private:
    ...
    ParticleTransformerFP32Algo algo_;
  };

}  // namespace ALPAKA_ACCELERATOR_NAMESPACE

DEFINE_FWK_ALPAKA_MODULE(ParticleTransformerFP32Producer);
